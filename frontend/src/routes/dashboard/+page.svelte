	...
	async function fetchFinancialData() {
		try {
			...
			const recentDonations = await pb.collection('donations').getList(1, 3, {
				sort: '-created',
				fields: 'donor_name,amount,currency,created'
			});
			
			const recentExpenses = await pb.collection('expenses').getList(1, 3, {
				sort: '-created',
				filter: 'status = "Approved"',
				fields: 'description,amount,currency,created,category'
			});
								...
								<div class="flex items-center">
									...
									<div class="ml-5 w-0 flex-1">
										<dl>
											...
											<dd>
												<div class="text-2xl font-bold text-green-900">${totalIncome.toLocaleString()}</div>
											</dd>
										</dl>
									</div>
								</div>
								...
								<div class="flex items-center">
									...
									<div class="ml-5 w-0 flex-1">
										<dl>
											...
											<dd>
												<div class="text-2xl font-bold text-red-900">${totalExpenses.toLocaleString()}</div>
											</dd>
										</dl>
									</div>
								</div>
								...
								<div class="flex items-center">
									...
									<div class="ml-5 w-0 flex-1">
										<dl>
											...
											<dd>
												<div class="text-2xl font-bold {currentBalance >= 0 ? 'text-blue-900' : 'text-red-900'}">${currentBalance.toLocaleString()}</div>
											</dd>
										</dl>
									</div>
								</div>
